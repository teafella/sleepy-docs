...
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null && pwd )"
    ...
    # else
        ...
        python3 lib/esptool/esptool.py --no-stub flash_id
        if python3 lib/esptool/esptool.py write_flash 0x0000 os/sleepy_binaries/h2/h2ctl/Latest.bin | grep -q "Could not connect"; then
            echo "Flashing Failed! Trying one more time."
            if python3 lib/esptool/esptool.py write_flash 0x0000 os/sleepy_binaries/h2/h2ctl/Latest.bin | grep "Could not connect"; then
                python3 lib/esptool/esptool.py --no-stub flash_id
